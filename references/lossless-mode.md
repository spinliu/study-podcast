...
- 一单元 ≈ **30 分钟音频 ≈ 8500 字**（中文朗读约 280–290 字/分钟；用 `chunk_book.py --chars-per-unit 8500`）。
- **只在自然段落边界切**，绝不切断句子。
...
续读指令(resume_cmd)：从「下一起点」那句开始，按相同 voice_params 朗读约 30 分钟（≈8500字），在自然段落边界收尾，然后写一个新的本标记并更新 resume_state.json。
